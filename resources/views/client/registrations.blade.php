...
@section('content')
    <main class="pt-32 pb-24 px-6 lg:px-12">
        <div class="max-w-[1440px] mx-auto">
            <section class="mb-12">
                <h1 class="text-4xl md:text-5xl font-black text-secondary uppercase italic leading-none tracking-tighter mb-4">
                            ...
                            <div class="flex-grow p-8 lg:p-10 flex flex-col md:flex-row justify-between items-center gap-8">
                                <div class="w-full">
                                    ...
                                    <div class="grid grid-cols-2 md:flex md:items-center gap-x-8 gap-y-2 mt-4">
                                        <div>
                                            <p class="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">Data</p>
                                            <p class="text-sm font-bold text-secondary">{{ $event->event_date->translatedFormat('d \d\e F, Y') }}</p>
                                        </div>
                                        <div>
                                            <p class="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">Localização</p>
                                            <p class="text-sm font-bold text-secondary">{{ $event->city }}, {{ $event->state }}</p>
                                        </div>
                                        <div>
                                            <p class="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">Kit Escolhido</p>
                                            <p class="text-sm font-bold text-secondary">{{ $category->name }} - {{ $category->distance }}</p>
                                        </div>
                                        <div>
