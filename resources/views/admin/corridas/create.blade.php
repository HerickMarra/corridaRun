        ...
        @if ($errors->any())
            <div class="bg-red-50 border border-red-200 rounded-xl p-4">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        <h4 class="text-sm font-bold text-red-800 mb-2">Erro ao criar corrida</h4>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2 space-y-1.5">
                            ...
                            <textarea name="description" required rows="4"
                                class="w-full bg-slate-50 border-transparent rounded-xl px-5 py-4 text-sm font-bold focus:bg-white transition-all"
                                placeholder="Descreva os detalhes da prova...">{{ old('description') }}</textarea>
                        </div>
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Data da
                                Prova</label>
                            <input name="event_date" value="{{ old('event_date') }}" required
                                class="w-full bg-slate-50 border-transparent rounded-xl px-5 py-4 text-sm font-bold focus:bg-white transition-all"
                                type="datetime-local" />
                        </div>
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Vagas
                                Totais</label>
                            <input name="max_participants" value="{{ old('max_participants') }}" required
                                class="w-full bg-slate-50 border-transparent rounded-xl px-5 py-4 text-sm font-bold focus:bg-white transition-all"
                ...
                <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
                    <h3 class="text-lg font-black uppercase italic tracking-tight mb-6 border-b border-slate-50 pb-4">
                        Tags do Evento</h3>
                    <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4 ml-1">Selecione as
                        categorias que definem esta prova (Ex: Trail, Street, MTB)</p>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        @foreach($tags as $tag)
                            <label class="relative group cursor-pointer">
                                <input type="checkbox" name="tags[]" value="{{ $tag->id }}" class="peer hidden">
                                <div
                                    class="px-4 py-3 rounded-xl border border-slate-100 bg-slate-50 text-slate-400 font-bold text-[10px] uppercase tracking-widest transition-all peer-checked:bg-white peer-checked:border-primary peer-checked:text-primary peer-checked:shadow-sm flex items-center gap-2 group-hover:border-primary/30">
                    ...
                    @if($tags->isEmpty())
                        <div class="p-4 rounded-xl bg-slate-50 border border-slate-100 text-center">
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest italic">Nenhuma tag
                                cadastrada. <a href="{{ route('admin.tags.index') }}" class="text-primary hover:underline"
                                    target="_blank">Criar agora</a></p>
                        </div>
                    @endif
                </div>
            ...
            <div class="lg:col-span-1 space-y-8">
                ...
                <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
                    ...
                    <div class="space-y-6">
                        <div class="space-y-1.5">
                            ...
                            <input name="registration_start" value="{{ old('registration_start') }}" required
                                class="w-full bg-slate-50 border-transparent rounded-xl px-5 py-4 text-sm font-bold focus:bg-white transition-all"
                                type="datetime-local" />
                        </div>
                        <div class="space-y-1.5">
                            ...
                            <input name="registration_end" value="{{ old('registration_end') }}" required
                                class="w-full bg-slate-50 border-transparent rounded-xl px-5 py-4 text-sm font-bold focus:bg-white transition-all"
                                type="datetime-local" />
                        </div>
                    </div>
                ...
                <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
                    <h3 class="text-lg font-black uppercase italic tracking-tight mb-6 border-b border-slate-50 pb-4">Status
                        da Corrida</h3>
                    <div class="space-y-1.5">
                        <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Estado
                            Atual</label>
                        <select name="status" required
                            ...
                            <option value="draft" {{ old('status') == 'draft' ? 'selected' : '' }}>Rascunho (Privado)</option>
                            <option value="published" {{ old('status', 'published') == 'published' ? 'selected' : '' }}>
                                Publicado (Visível)</option>
                            <option value="closed" {{ old('status') == 'closed' ? 'selected' : '' }}>Encerrado (Inscrições
                                Suspensas)</option>
                    ...
                    <div class="mt-6 p-4 bg-slate-50 rounded-xl border border-slate-100 flex items-center gap-3">
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="ignore_fees" value="1" class="sr-only peer" {{ old('ignore_fees') ? 'checked' : '' }}>
                            <div
                                class="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary">
                            </div>
                        </label>
                        <span class="text-[10px] font-black uppercase tracking-widest text-slate-600">Isentar de
                            Taxas</span>
                    </div>
                </div>
                ...
                <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-1.5">
                            <label
                                class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Alimentação</label>
                            <select name="nutrition"
                                class="w-full bg-slate-50 border-transparent rounded-xl px-5 py-4 text-sm font-bold focus:bg-white transition-all cursor-pointer">
                                <option value="not_informed" {{ old('nutrition', 'not_informed') == 'not_informed' ? 'selected' : '' }}>Não Informado</option>
                                <option value="none" {{ old('nutrition') == 'none' ? 'selected' : '' }}>Não Fornecido</option>
                                <option value="partial" {{ old('nutrition') == 'partial' ? 'selected' : '' }}>Parcial</option>
                                <option value="complete" {{ old('nutrition') == 'complete' ? 'selected' : '' }}>Completo
                                </option>
                            </select>
                            <p class="text-[9px] text-slate-400 mt-2 font-medium">Tipo de alimentação disponível durante a
                                prova.</p>
                        </div>
                        <div class="space-y-1.5">
                            <label
                                class="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Hidratação</label>
                            <select name="hydration"
                                class="w-full bg-slate-50 border-transparent rounded-xl px-5 py-4 text-sm font-bold focus:bg-white transition-all cursor-pointer">
                                <option value="not_informed" {{ old('hydration', 'not_informed') == 'not_informed' ? 'selected' : '' }}>Não Informado</option>
                                <option value="none" {{ old('hydration') == 'none' ? 'selected' : '' }}>Não Fornecido</option>
                                <option value="partial" {{ old('hydration') == 'partial' ? 'selected' : '' }}>Parcial</option>
                                <option value="complete" {{ old('hydration') == 'complete' ? 'selected' : '' }}>Completo
                                </option>
                            </select>
                            <p class="text-[9px] text-slate-400 mt-2 font-medium">Tipo de hidratação disponível durante a
                                prova.</p>
                        </div>
                    </div>
                </div>
