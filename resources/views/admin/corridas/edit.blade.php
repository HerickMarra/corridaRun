    ...
    <form action="{{ route('admin.corridas.update', $event->id) }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
                    <h3 class="text-lg font-black uppercase italic tracking-tight mb-6 border-b border-slate-50 pb-4">
                        Regulamento da Prova</h3>
                    <div id="regulation-editor" class="h-64">
                        {!! $event->regulation !!}
                    </div>
                    <input type="hidden" name="regulation" id="regulation-input" value="{{ $event->regulation }}">
                </div>
                            ...
                            <div class="category-item p-6 rounded-2xl bg-slate-50 border border-slate-100 relative group">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                    ...
                                    <div class="space-y-1.5 lg:col-span-4">
                                        ...
                                        <textarea name="categories[{{ $index }}][items_included]"
                                            class="w-full bg-white border-transparent rounded-lg px-4 py-3 text-xs font-bold"
                                            placeholder="Ex: Medalha, Camiseta, Chip, Hidratação"
                                            rows="2">{{ is_array($category->items_included) ? implode(', ', $category->items_included) : '' }}</textarea>
                                    </div>
                                </div>
                ...
                <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
                    ...
                    <div id="routes-container" class="space-y-6">
                        @foreach($event->routes as $index => $route)
                            <div
                                class="route-item p-6 rounded-2xl bg-slate-50 border border-slate-100 relative group animate-in slide-in-from-top-4 duration-500">
                                <input type="hidden" name="routes[{{ $index }}][id]" value="{{ $route->id }}">
                                <button type="button"
                                    class="remove-route absolute -right-2 -top-2 size-6 bg-red-500 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all shadow-lg">
                                    <span class="material-symbols-outlined text-xs">close</span>
                                </button>
                                <div class="grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
                                    <div class="md:col-span-5 space-y-1.5">
                                        <label class="text-[10px] font-black uppercase tracking-widest text-slate-400">Nome do
                                            Trajeto</label>
                                        <input name="routes[{{ $index }}][name]" value="{{ $route->name }}" required
                                            class="w-full bg-white border-transparent rounded-xl px-4 py-3 text-xs font-bold"
                                            placeholder="Ex: Percurso 5KM" type="text" />
                                    </div>
                                    <div class="md:col-span-3 space-y-1.5">
                                        <label class="text-[10px] font-black uppercase tracking-widest text-slate-400">Cor da
                                            Linha</label>
                                        <input name="routes[{{ $index }}][color]" value="{{ $route->color }}"
                                            class="route-color-input w-full h-11 bg-white border-transparent rounded-xl px-1 py-1 cursor-pointer"
                                            type="color" data-index="{{ $index }}" />
                                    </div>
                                    <div class="md:col-span-4">
                                        <button type="button"
                                            class="draw-route-btn w-full bg-slate-800 text-white py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-primary transition-all"
                                            data-index="{{ $index }}">
                                            <span class="material-symbols-outlined text-sm">edit_location_alt</span>
                                            Editar no Mapa
                                        </button>
                                    </div>
                                    <input type="hidden" name="routes[{{ $index }}][path]" id="route-path-{{ $index }}"
                                        value="{{ json_encode($route->path) }}" class="route-path-input" />
                                    <input type="hidden" name="routes[{{ $index }}][markers]" id="route-markers-{{ $index }}"
                                        value="{{ json_encode($route->markers) }}" class="route-markers-input" />
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="lg:col-span-1 space-y-8">
                ...
                <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <p class="text-[9px] text-slate-400 mt-2 font-medium">Controle a visibilidade e o estado das
                            inscrições.</p>
                    </div>

                    <div class="mt-6 p-4 bg-slate-50 rounded-xl border border-slate-100 flex items-center gap-3">
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="ignore_fees" value="1" class="sr-only peer" {{ old('ignore_fees', $event->ignore_fees) ? 'checked' : '' }}>
                            <div class="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"></div>
                        </label>
                        <span class="text-[10px] font-black uppercase tracking-widest text-slate-600">Isentar de Taxas</span>
                    </div>

                    <div class="mt-4 p-4 bg-slate-50 rounded-xl border border-slate-100 flex items-center gap-3">
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="allow_user_refund" value="1" class="sr-only peer" {{ old('allow_user_refund', $event->allow_user_refund) ? 'checked' : '' }}>
                            <div class="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"></div>
                        </label>
                        <span class="text-[10px] font-black uppercase tracking-widest text-slate-600">Permitir Devolução (Usuário)</span>
                    </div>
                </div>
...
@push('scripts')
    ...
    <script>
        ...
        let routeIndex = {{ $event->routes->count() }};
        ...
        function reindexRoutes() {
            ...
            items.forEach((item, index) => {
                ...
                const markerInput = item.querySelector('.route-markers-input');
                if (markerInput) markerInput.id = `route-markers-${index}`;
            });
        }
        ...
        function initMap() {
            ...
            const existingRoutes = @json($event->routes);
            existingRoutes.forEach((route, idx) => {
                ...
                const markers = route.markers || { start: null, end: null, both: null };
                const markerObjects = { start: null, end: null, both: null };
                ...
                Object.keys(markers).forEach(type => {
                    if (markers[type]) {
                        ...
                        markerObjects[type] = new google.maps.Marker({
                ...
                // Center map on first route if exists
