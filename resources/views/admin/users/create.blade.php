    ...
    <div class="max-w-4xl mx-auto">
        ...
        <form action="{{ route('admin.users.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div id="events-selection-section"
                    class="md:col-span-2 bg-white p-8 rounded-2xl shadow-sm border border-slate-100 {{ old('role') === 'organizador' ? '' : 'hidden' }}">
                    <div
                        class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 border-b border-slate-50 pb-4">
                        <div>
                            <h3 class="text-lg font-black uppercase italic tracking-tight">Provas Vinculadas</h3>
                            <p class="text-slate-500 text-xs font-medium italic">Selecione as corridas que este organizador
                                poderá gerenciar.</p>
                        </div>
                    ...
                    <div class="max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                        <div id="events-grid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            @php
                                $selectedEvents = old('events', []);
                            @endphp
                            @foreach($events as $event)
                                <label
                                    class="event-selection-card flex items-center gap-4 p-3 bg-slate-50 rounded-2xl border-2 border-transparent hover:border-primary/30 transition-all cursor-pointer group relative overflow-hidden"
                                    ...
                                    <div class="flex flex-col flex-grow min-w-0">
                                        <span
                                            class="text-sm font-bold text-slate-800 truncate uppercase italic tracking-tight group-hover:text-primary transition-colors">{{ $event->name }}</span>
                                        <div class="flex items-center gap-1.5 mt-0.5">
                                            <span
                                                class="material-symbols-outlined text-[10px] text-primary">calendar_today</span>
                                            <span
                                                class="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{{ $event->event_date->format('d/m/Y') }}</span>
                                        </div>
                                        <div class="flex items-center gap-1.5">
                                            <span
                                                class="material-symbols-outlined text-[10px] text-slate-300">location_on</span>
                                            <span
                                                class="text-[10px] font-bold text-slate-400 uppercase tracking-tighter truncate">{{ $event->city }}/{{ $event->state }}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div
                                        class="absolute inset-0 border-2 border-primary rounded-2xl pointer-events-none opacity-0 transition-opacity {{ in_array($event->id, $selectedEvents) ? 'opacity-100' : '' }} checked-border">
                                    </div>
                                </label>
                            @endforeach
                        </div>
                        ...
                        <div id="no-events-found" class="hidden py-20 text-center">
                            ...
                            <p class="text-slate-400 font-bold uppercase tracking-widest text-xs">Nenhuma corrida encontrada
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <script>
                ...
                // Storage for checked events
                const checkedEvents = new Map();

                // Initialize checkedEvents map
