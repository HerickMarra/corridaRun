...
@section('content')
    ...
    <div class="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm max-w-2xl">
        <form action="{{ route('admin.service-fees.update', $service_fee->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div>
                    <label for="name"
                        ...
                        Taxa</label>
                    <input type="text" name="name" id="name" value="{{ old('name', $service_fee->name) }}" required
                ...
                <div class="grid grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="type" id="type" required
                            class="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-sm font-bold text-slate-700">
                            <option value="fixed" {{ old('type', $service_fee->type) == 'fixed' ? 'selected' : '' }}>Valor
                                Fixo (R$)</option>
                            <option value="percentage" {{ old('type', $service_fee->type) == 'percentage' ? 'selected' : '' }}>Percentual (%)</option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="value" id="value" value="{{ old('value', $service_fee->value) }}"
                            step="0.01" required
                            class="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-sm font-bold text-slate-700"
                            placeholder="0.00">
                ...
                <div class="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    <div class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ old('is_active', $service_fee->is_active) ? 'checked' : '' }}>
                        <div
